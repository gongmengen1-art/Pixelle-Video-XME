...
- ✅ **2026-06-15**: Custom Media mode subtitle & voice upgrade — (1) **punctuation-free subtitles** — the voiceover still reads the original punctuated text (for correct phrasing), while the on-screen subtitle drops sentence-end / pause punctuation for a cleaner look; (2) **emphasis highlighting** — wrap words in `[[ ]]` and they show up colored + enlarged + bold in the subtitle, with a configurable emphasis color; (3) **VoxCPM voice cloning** — besides the Edge preset voices, upload a reference clip to clone its voice (runs on RunningHub; the reference transcript is auto-recognized, no manual typing)
- ✅ **2026-06-09**: Custom Media mode stability & quality overhaul — (1) fixed a single network hiccup (Edge TTS connection reset) aborting the entire generation; connection errors now auto-retry with exponential backoff; (2) fixed the finished video disappearing when interacting with the page (adding paragraphs, changing durations, etc.) — the result is now persisted in session state and only the latest generation is kept; (3) major speedup of per-scene subtitle rendering (transparent overlay + dropping the networkidle wait + concurrent rendering), cutting a long-script scene from ~10 minutes to ~1 minute; (4) fidelity fix — removed redundant lossy intermediate encoding (near-lossless intermediates + final CRF 18 + lanczos scaling), so the output is visibly closer to the source assets; (5) fixed "uploading N assets but only some appear" — same-named files no longer overwrite each other, and when there are more assets than script segments, a segment is now split sentence-by-sentence across multiple assets so every uploaded asset appears in the final video
...
- ✅ **Subtitle Emphasis Highlighting** - Mark key words with `[[ ]]` in the script; they render colored / enlarged / bold in the subtitle, with a configurable emphasis color; punctuation is omitted from the subtitle (without affecting the voiceover)
- ✅ **Voice Cloning** - Beyond the Edge multilingual preset voices, upload a reference clip to clone its voice (VoxCPM, runs on RunningHub with auto-transcription of the reference)
