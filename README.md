...
- ✅ **2026-06-15**: 自定义素材模式字幕与配音升级——①字幕**去标点显示**（配音仍读带标点原文保证断句发音，成片字幕自动省略句末/停顿标点，画面更干净）；②**重点字高亮**——文案中用 `[[ ]]` 包住的字会在字幕中变色 + 放大 + 加粗，重点色可在界面自定义；③接入 **VoxCPM 声音克隆**配音——除 Edge 预设音色外，可上传一段参考音频克隆其音色（经 RunningHub 云端运行，参考音频内容自动识别转写，无需手填）
- ✅ **2026-06-09**: 自定义素材模式稳定性与质量大幅优化——①修复一次网络抖动（Edge TTS 连接被重置）就导致整条生成失败的问题，连接类错误改为自动指数退避重试；②修复生成好的成片在操作页面（增删段落、调整时长等）时丢失——成片结果持久化到会话状态，多次生成只保留最新一条；③单场景字幕渲染大幅提速（透明叠加层 + 去除 networkidle 等待 + 并发渲染），长文案单场景从约 10 分钟降到 1 分钟级；④画质保真——消除中间环节的冗余有损编码（近无损中间件 + 最终 CRF 18 + lanczos 缩放），成片清晰度明显贴近原始素材；⑤修复「上传 N 个素材成片只出现一部分」——同名文件不再相互覆盖，且当素材数多于文案段数时，一段文案自动按句拆分跨多个素材，确保所有上传素材都出现在成片
- ✅ **2026-06-07**: 「一键发布」升级——新增微信视频号；多平台合并为**单个一键发布按钮**（依次为各平台打开浏览器并填好，三窗同时挂起、统一等待用户手动点击发布）；修复未点发布直接关闭浏览器时客户端抛异常，改为「已取消发布」状态正常展示
- ✅ **2026-06-02**: 自定义素材模式语音与画面质量优化——文案改为「整段连续 TTS」彻底消除句中断句（基于 edge-tts 字级时间戳精确对齐字幕，ComfyUI 后端自动按字数比例兜底）；修复短素材循环时的画面闪烁（循环前统一归一化 fps/时基/SAR，循环接缝交叉淡入收窄到 0.25s）
- ✅ **2026-06-01**: 新增「一键发布」模块，半自动发布视频到抖音/小红书——程序自动填好标题/话题并上传视频，最后【发布】按钮由用户手动点击；扫码登录一次后复用登录态，仅本地运行
- ✅ **2026-05-29**: 自定义素材模式新增封面上传功能（为后续一键发布抖音/小红书做准备）；修复"文案长于视频素材时画面冻结"问题——短素材自动循环并在循环点做 0.5s 交叉淡入
- ✅ **2026-05-28**: 自定义素材模式全面升级：新增字幕精准同步引擎、多段文案"+"交互、字幕样式/位置独立配置；修复视频素材拼接卡顿问题
...
- ✅ **字幕精准同步** - 自定义素材模式下，每段文案独立 TTS + 字幕渲染；长文案自动按标点切分，字幕与配音逐句对齐
- ✅ **多段文案自由配置** - 自定义模式支持"+"号添加多段文案，每段独立选择字幕样式（简洁白字、港式黄红、描边、半透背景、卡片等）
- ✅ **字幕样式与位置** - 支持字幕上/中/下位置、每行字数、最大行数独立配置；样式通过 CSS 预设注入，扩展无需改模板
- ✅ **字幕重点高亮** - 文案中用 `[[ ]]` 标记重点字，字幕显示时自动变色 / 放大 / 加粗，重点色可在界面自定义；标点自动从字幕画面中省略（不影响配音）
- ✅ **声音克隆配音** - 除 Edge 多语言预设音色外，支持上传参考音频克隆音色（VoxCPM，经 RunningHub 云端运行，参考音频自动转写）
- ✅ **自定义封面上传** - 自定义素材模式支持上传封面图片，输出与视频同目录（`{task_id}_cover.{ext}`），并写入 `metadata.json`，为一键发布抖音/小红书等平台预留接入点
- ✅ **智能视频循环** - 当文案时长超过视频素材时长时，自动循环源素材并在循环边界做 0.5s 交叉淡入，画面永不冻结；视频长于文案时按文案截断
- ✅ **一键发布到抖音/小红书/微信视频号** - 独立「📤 一键发布」标签页，基于 Playwright 浏览器自动化；多平台**一键发布**：依次为各平台打开浏览器并自动填写标题/话题、上传视频，所有窗口同时挂起，由用户在浏览器中逐个手动点击【发布】；扫码登录一次后复用登录态（本地保存），未点发布直接关窗显示为「已取消发布」而非报错；新增平台只需继承 `BasePublisher`
